...
const FormNote =async( )=>{
    ...
    const Datas = {
        list    : JSON.parse( localStorage.getItem( 'lstNotes' )),
        list2   : [],
        data    : {
    ...
    if( typeForm === 'add' || typeForm === 'offline' ){ 
        ...
        const { offline = [] } = Datas.list 
        const data  = JSON.parse( JSON.stringify( Datas.data ) ) 
        

        Datas.list2 = offline.filter((data)=> data.id !== Datas.data.id && data )
        Datas.data  = offline.filter((data)=> data.id === Datas.data.id && data ) 
    ...
    element.addEventListener( 'input', ()=>{
        ...
        Datas.list.offline = data.title === '' && data.details === '' ? Datas.list2 : [ ...Datas.list2, data ] 
        localStorage.setItem( 'lstNotes', JSON.stringify( Datas.list ) )
    } )
